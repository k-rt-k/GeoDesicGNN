...
def main(params):
    ...
    edges = []
    with open(
        osp.join(params.data_folder_path, params.train_data_set + ".csv"), "r"
    ) as f:
        lines = f.readlines()
        for line in lines:
            ...
            for n in edge:
                if n not in name2id:
                    name2id[n] = train_num_entities
                    train_num_entities += 1
                edge_id.append(name2id[n])
...
if __name__ == "__main__":
    ...
    parser.add_argument("--train_data_set", type=str, default="citeseer")
